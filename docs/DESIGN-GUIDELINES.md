...
|---|---|---|---|---|
| 2026-05-01 | Hero scene movement | Static hero with bottom-left blue glow and no animated texture | Scan line overlay added: static horizontal lines at 4px intervals (rgba(255,255,255,0.03), aria-hidden, pointer-events-none) + single pulsing scan band (60px tall, rgba(0,102,255,0.06) center fading to transparent at edges, animates top from -60px to 100% over 8s linear infinite). Both layers sit at z-1, between the background blue glow (z-auto) and content (z-2). Pulse band respects prefers-reduced-motion (suppressed); static lines always render. | Feedback indicated the Hero felt static. Tested several treatments in a localhost sandbox — scan lines won because they reference video equipment language (broadcast monitors, viewfinder overlays) without being literal, and add atmosphere without the mobile data cost of background video. Tested cursor-reactive variants and ruled them out — felt too interactive for a hero that's meant to be ambient. Tested two scan line intensities (subtle and stronger) — subtle won because the hero's job is to frame the headline, not compete with it. Static lines are always-on cinema atmosphere; pulse band adds slow vertical movement without demanding attention. |
| 2026-05-01 | About scene movement | Static portrait with no scroll-triggered animation | Color grade transition: AboutPortraitCard renders initially in monochrome (grayscale(1) brightness(0.65) contrast(1.1)), then transitions to full color (grayscale(0) brightness(1) contrast(1)) over 3.5s ease-out when the card enters viewport (IntersectionObserver, threshold 0.3, fires once per session). Filter applies to the entire card so the viewfinder overlays (REC dot, timecode, corner brackets, blue radial glow) settle into full color alongside the portrait — reads as "color grade emerging" rather than just photo desaturation. Respects prefers-reduced-motion by rendering at the final color state immediately. | Feedback indicated the About scene felt static. Tested 5 movement directions in a localhost sandbox; color grade transition won because it references Jack's actual cinematography craft (color grading the footage) and lands as a cinematic gesture rather than a generic web animation. Timing tested at 2.5s (default), 3.5s, and "hold 1s then 2.5s transition" — 3.5s ease-out felt right: long enough that the grade emergence reads as deliberate, short enough that users on a quick scroll-through still see it complete. The whole-card filter scope was chosen over <Image>-only because the unified settling of the viewfinder UI from desaturated to full reads as the look settling in, reinforcing the color grade metaphor. |
| 2026-05-01 | About scene body copy | Two-paragraph mockup-sourced placeholder bio ("A Trinidad-based cinematographer... / Nine years behind the lens..."). Generic editorial framing, no brand-client name-drops, no kit specifics. | Four-paragraph Nathan-authored bio: drone-and-FPV-specialist positioning (origin 2018, Jack Visuals launch 2024), explicit client roster (Patrón, Grey Goose, Jack Daniel's, Gin Mare and Diplomático, plus T&T carnival events), kit specifics (Sony A7 IV, DJI Mini 5 Pro, DJI Avata 2, DJI Ronin 5), and a CTA closer ("Ready to bring your vision to life? Let's create something worth watching."). British spelling ("recognised") and apostrophes preserved verbatim per Nathan's draft. | The mockup copy was placeholder drafted before Nathan had agency over the voice. This is his official bio. Three editorial gains worth noting: (a) drone-FPV positioning matches his actual practice rather than generic "cinematographer," (b) the named-brand list reinforces the Trusted By wall directly above — all 5 brands appear there, so the bio's claims are immediately verifiable, (c) the CTA closer ("Let's create something worth watching") bridges editorially toward the Inquire scene at the bottom of the page. Together with the MEET NATHAN. → MEET JACK. headline change filed earlier today, this makes the About scene Nathan's voice end-to-end. |
